...
download_directory= fr'{os.getcwd()}\{todate}'
...
class UseSel(object):
    def __init__(self):
        try:
            ...
            pass
        try:
            os.mkdir(fr'{os.getcwd()}\winpydriver')
            print('下記フォルダにWebdriverを配置してください')
            print(fr'{os.getcwd()}\winpydriver')
        except:
            pass
        envpath = fr'{os.getcwd()}\winpydriver'
        orgpath = os.environ['PATH']
        if orgpath.find(envpath) == -1:
            os.environ['PATH'] = f'{envpath};{orgpath}'
    ...
    def log(
        ...
        **args
        ...
        dpath = fr'{os.getcwd()}\winpydriver'
        try:
            if browser == "ff" or browser == "firefox":
                ...
                ffwdpath = fr"{dpath}\geckodriver.exe"
                ...
                driver = webdriver.Edge(fr"{dpath}\MicrosoftWebDriver.exe")
                ...
                driver = webdriver.Chrome(executable_path=fr"{dpath}\chromedriver.exe",chrome_options=options)
                ...
                driver = webdriver.Chrome(chrome_options=options,executable_path=fr"{dpath}\chromedriver.exe")
            elif browser == "ie" or browser == "internetexplorer" or browser == "internet":
                driver = webdriver.Ie(fr"{dpath}\IEDriverServer.exe")
            else:
